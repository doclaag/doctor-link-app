...
export const AppointmentTimeScreen = () => {
  ...
  const handleConfirm = ({ hours, minutes }: { hours: number, minutes: number }) => {
    ...
    const formattedHours = hours < 10 ? `0${hours}` : hours > 12 ? `0${hours - 12}` : hours; // Formato de 12 horas
    const period = hours >= 12 ? 'PM' : 'AM'; // AM o PM
    const formattedMinutes = minutes < 10 ? `0${minutes}` : minutes;
  7
    // Establece la hora sel7eccionada en el estado y oculta el selector de tiempo
    setSelectedTime(`${formattedHours}:${formattedMinutes} ${period}`); // Agrega el período (AM o PM)
    hideTimePicker();
  };
  

  const handleDateSelect = (day: any) => {
    const selectedDate = moment(day.dateString || day.date).locale('es').format('dddd DD [de] MMMM [del] YYYY'); // Formato de fecha en español
